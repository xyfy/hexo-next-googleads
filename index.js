...
hexo.extend.filter.register('theme_inject', injects => {
  ...
  if (!config.ads_txt) {
    ...
    if (fs.exists(adstxtPath)) {
      let content = fs.readFile(adstxtPath);
      if (!content.includes(config.ads_txt)) {
        fs.appendFile(config.ads_txt)
      }
    }
    else {
      fs.writeFile(adstxtPath, config.ads_txt);
    }
  }
